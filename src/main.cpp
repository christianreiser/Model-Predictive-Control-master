    ...
    if (sdata.size() > 2 && sdata[0] == '4' && sdata[1] == '2') {
      ...
      if (s != "") {
        ...
        if (event == "telemetry") {
          ...
          v = v * 0.447; // from mph to km/h
          double steering_angle = j[1]["steering_angle"];
          double throttle = j[1]["throttle"];
          


          int N = ptsx.size(); // # of waypoints
          ...
          */
          ...
          // Convert from map to the vehicle coordinate system
          VectorXd x_veh(N);
          VectorXd y_veh(N);
          ...
          double cte = coeffs[0]; //cross track error
          ...
          // SUBMITTING.
          this_thread::sleep_for(chrono::milliseconds(int(dt*1000))); //s to ms

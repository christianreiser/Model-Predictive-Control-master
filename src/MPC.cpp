...
class FG_eval {
 public:
  ...
  void operator()(ADvector& fg, const ADvector& vars) {
    ...
    // added 1 to each of the starting indices due to 
    ...
    for (int i = 0; i < N - 1; i++) {
      ...
      AD<double> f0 = 0.0; //polynomial value
      for (int i = 0; i < coeffs.size(); i++) {
        f0 += coeffs[i] * CppAD::pow(x0, i);
      }
      AD<double> psides0 = 0.0;
      for (int i = 1; i < coeffs.size(); i++) {
        psides0 += i*coeffs[i] * CppAD::pow(x0, i-1);  //desired psi
      }
      psides0 = CppAD::atan(psides0); //desired psi
...
vector<double> MPC::Solve(Eigen::VectorXd state, Eigen::VectorXd coeffs) {
  ...
  //
  // 6 due to x,y,psi,v,CTE,epsi
  // 2 due to Delta and acceleration a
  // N-1 because there is no delta (and a) at the end
  ...
  //set initial values for first state
